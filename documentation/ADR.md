...
- **Mise à jour post-[DEC-043](#dec-043) et vérification 20/08/2026** : le pivot vers un provider Vision multimodal ouvre techniquement le support WebP, car Mistral Vision accepte nativement ce format. Cependant, le blocage WebP reste appliqué au niveau du contrôleur API (`RecipeController.CreateScannedRecipe`), qui filtre les uploads via une whitelist stricte JPEG et PNG (extensions, MIME types, magic bytes). Le support WebP end-to-end nécessite deux évolutions coordonnées : l'assouplissement de la whitelist contrôleur, et la mise en place d'une conversion serveur WebP vers PNG pour le path fallback OCR Tesseract (qui ne peut pas décoder WebP sans libwebp). Ces deux évolutions sont regroupées dans un nouveau ticket post V1, BACK-XXX "Support WebP end-to-end contrôleur API et conversion fallback OCR". En attendant, la DEC-025 reste strictement active, WebP est refusé quel que soit le provider actif.
