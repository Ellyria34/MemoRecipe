...
Le serveur applique 6 headers de sécurité via un `SecurityHeadersMiddleware` custom (X-Content-Type-Options, X-Frame-Options, Referrer-Policy, Permissions-Policy, CSP adapté à Blazor WASM et MudBlazor, HSTS conditionnel prod), plus une politique CORS externalisée dans `appsettings.json` avec fail-fast au démarrage si absente. La CSP inclut `'unsafe-inline'` sur `script-src` (imposé par la balise `<script type="importmap">` de Blazor WASM `.NET 8+`) et sur `style-src` (imposé par MudBlazor), appliquée à l'identique côté nginx front pour uniformité front/API. Kestrel est hardened au démarrage (`MaxRequestBodySize = 15 Mo` pour plafonner les uploads au niveau transport, `AddServerHeader = false` pour l'anti-fingerprinting OWASP). Un `ForwardedHeaders` middleware en tête de pipeline propage l'IP réelle du client et le schéma HTTPS depuis le reverse proxy edge, ce qui permet au rate limiter par IP de fonctionner correctement.
...
Le 27/08/2026 ([DEC-063](ADR.md#dec-063)), amendement de DEC-021 lors de la livraison P0-5 (security headers nginx front) : ajout de `'unsafe-inline'` dans la directive `script-src` de la CSP, appliqué simultanément côté API middleware et côté nginx front pour cohérence uniforme. Compromis rendu obligatoire par la balise `<script type="importmap">` de Blazor WASM `.NET 8+` (mécanisme standard non-contournable de fingerprinting des modules). Sans ce token, Blazor ne charge pas (bloqué par CSP). Le compromis sécu (XSS pourrait exécuter des inline scripts) reste acceptable au regard des autres couches de défense (validation inputs FluentValidation, cookies HttpOnly, rate limiting double couche, upload defense-in-depth, etc.). Migration vers CSP nonce-based envisagée V2.

**DEC détaillées** : [DEC-005](ADR.md#dec-005), [DEC-008](ADR.md#dec-008), [DEC-014](ADR.md#dec-014), [DEC-015](ADR.md#dec-015), [DEC-020](ADR.md#dec-020), [DEC-021](ADR.md#dec-021), [DEC-022](ADR.md#dec-022), [DEC-023](ADR.md#dec-023), [DEC-024](ADR.md#dec-024), [DEC-037](ADR.md#dec-037), [DEC-054](ADR.md#dec-054), [DEC-055](ADR.md#dec-055), [DEC-056](ADR.md#dec-056), [DEC-057](ADR.md#dec-057), [DEC-058](ADR.md#dec-058), [DEC-063](ADR.md#dec-063).
