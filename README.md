...
| **Tests** | Unit tests on validators / services / AI pipeline (deterministic fakes); integration tests via `WebApplicationFactory<Program>` with TestContainers (real PostgreSQL); end-to-end tests with Playwright — 6 scenarios covering auth, recipe CRUD and the full AI scan flow, run against the complete Docker Compose stack |
| **CI/CD** | GitHub Actions: build + tests on push and PR (API + IA + Web), end-to-end Playwright suite against an ephemeral Docker Compose stack, vulnerable-package scan (fail-fast on High/Critical), CodeQL SAST (C# + workflows), Lighthouse a11y/perf audit; container images pushed to GHCR on version tags |
